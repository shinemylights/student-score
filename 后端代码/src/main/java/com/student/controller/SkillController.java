...
public class SkillController {
    ...
    @ApiOperation(value = "新增技能特长")
    public Result<Skill> insert(@RequestBody Skill skill) {
        // StudentEvaluate se = evaluateUtils.getMyStudentEvaluate();
        // competition.setEvaluateId(se.getId());
        StUser currUser = (StUser) SecurityUtils.getSubject().getPrincipal();

        skill.setStudentId(currUser.getUsername());
        skill.setStudentName(currUser.getRealName());
        skill.setProofImageUrl(skill.getProofImageUrl().substring(1));

        skillService.save(skill);
        return new ResultUtil<Skill>().setData(skill);
    }
    ...
    public Result<IPage<Skill>> getMyPage(@RequestBody PageVo page){
        ...
        IPage<Skill> data = skillService.page(PageUtil.initMpPage(page), qw);

        return new ResultUtil<IPage<Skill>>().setData(data);
