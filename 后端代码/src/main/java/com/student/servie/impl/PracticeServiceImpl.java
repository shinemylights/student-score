...
import com.student.dao.mapper.PracticeMapper;
...
public class PracticeServiceImpl extends ServiceImpl<PracticeMapper, Practice>
    ...
    public Result getAuditList(AuditRequest request) {
        ...
        IPage<Practice> page=new Page<>(request.getCurrent(),pageSize);

        IPage<Practice> pageResult = this.page(page,new QueryWrapper<Practice>()
     ...
     */
    private AuditVO convertToAuditVO(Practice practice) {
        AuditVO auditVO = new AuditVO();
        // 假设 Contest 和 AuditVO 有一些共通的属性，可以使用 BeanUtils 复制属性
        BeanUtils.copyProperties(practice, auditVO);
    ...
    public Result getAuditById(int id) throws IOException {
        ...
        BeanUtils.copyProperties(practice, auditDetailVO);
        auditDetailVO.setImages(new ArrayList<>());

        String[] splitUrl = practice.getProofImageUrl().split(",");
