...
export default {
  ...
  methods:{
    async approveAward(){
      const res= await this.$axios.get('/audit/skill/approveAward?id=' + this.$route.query.id)
    ...
    async rejectAward(){
      try {
        const res= await this.$axios.get('/audit/skill/rejectAward?id=' + this.$route.query.id)
        if(res.data.code===200){
          // this.adminData={}
          // this.adminDialog=false
          // await this.getReviewById()
    ...
    async getReviewById(){
      const res= await this.$axios.get('/audit/skill/getAuditById?id=' + this.$route.query.id)
